...
export const preventDeleteWhenConsumedHook: CollectionBeforeDeleteHook = async ({
  ...
  try {
    ...
    if (usage && usage.length > 0) {
      throw new APIError(`Cannot delete an item currently in use`, 400)
    }
  } catch (error) {
...
async function getUsageData(payload: BasePayload, collection: CollectionSlug, id: string | number) {
  try {
